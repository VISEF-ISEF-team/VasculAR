...
class CTkCalendar(ctk.CTkFrame):
    ...
    def setup_button_normal(self, frame, day, row, column):
        ...
        if self.today_fg_color is not None and self.date_is_today((day, self.month, self.year)):
            button = ctk.CTkButton(frame, text=str(day), corner_radius=5,
                                   fg_color=self.btn_color, font=ctk.CTkFont("Arial", 11),
        ...
        button.grid(row=row, column=column, sticky="nsew", padx=self.calendar_label_pad, pady=self.calendar_label_pad)
        ...
        for row in range(len(current_month)):
            for column in range(7):
                if current_month[row][column] != 0:
                    self.setup_button_normal(calendar_frame, current_month[row][column], row, column)
    ...
    def setup_label_normal(self, frame, day, row, column):
        ...
        label.grid(row=row, column=column, sticky="nsew", padx=self.calendar_label_pad, pady=self.calendar_label_pad)
...
class DatabaseManagment(customtkinter.CTk):
    ...
    def create_layouts(self):
        ...
        self.label = customtkinter.CTkLabel(master=self.frame, text="Cloud Database Management", font=(self.font, 25, "bold"))
        self.label.grid(row=0, column=0, padx=20, pady=10, sticky='w')
        
        calendar_icon = customtkinter.CTkImage(dark_image=Image.open("imgs/calendar.png"),size=(20, 20))
        self.calender_entry = customtkinter.CTkButton(master=self.frame, text='', image=calendar_icon, width=30, height=30, command=self.open_calender)
        ...
        self.entry.bind("<KeyRelease>", lambda e: self.search_data(name_patient=self.entry.get()))
        
        self.about_button = customtkinter.CTkButton(master=self.frame, text="?", hover=False, width=30, command=self.open_about_window)
    ...
    def info_window(self, key, value):
        ...
        for (analysis_key, analysis_value), (canvas, image_url) in zip(value['analysis_data'].items(), value['folder_imgs'][key].items()):
            ...
            if analysis_key == canvas:
                ...
                my_image = customtkinter.CTkImage(dark_image=Image.open(img_data), size=(350, 350))

                image_label = customtkinter.CTkLabel(self.info_frame, image=my_image, text="") 
                image_label.pack()
                image_label.image = my_image
                
                # display description
                key_label = customtkinter.CTkLabel(self.info_frame, text=list(analysis_value.keys())[0], wraplength=300, font=(self.font, 15, "bold"))
                key_label.pack(padx=10, pady=(10,5))
                
                des_label = customtkinter.CTkLabel(self.info_frame, text=list(analysis_value.values())[0]  , wraplength=300)
