...
def show_sitk_img_info(img: sitk.Image):
  ...
  rounded_spacing = tuple(round(x, 3) for x in spacing)
  rounded_origin = tuple(round(x, 3) for x in origin)
  
  comma_separated_origin = ',  '.join(str(x) for x in rounded_origin)
  comma_separated_spacing = ',  '.join(str(x) for x in rounded_spacing)
  comma_separated_dimensions = ',  '.join(str(x) for x in dimensions)

  info = {'Pixel Type' : pixel_type, 'Dimensions': comma_separated_dimensions, 'Spacing': comma_separated_spacing, 'Origin': comma_separated_origin,  'Direction' : direction[0:4], '' :direction[4:9] }
  return info
